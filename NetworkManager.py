...
class SignalDispatcher(object):
    ...
    def listen_for_restarts(self):
        ...
        if not NMDbusInterface.last_disconnect and dbus.get_default_main_loop():
            dbus.SystemBus().add_signal_receiver(
                self.handle_restart, "NameOwnerChanged", "org.freedesktop.DBus"
            )
    ...
    def handle_signal(self, *args, **kwargs):
        ...
        try:
            ...
            for arg, (name, signature) in zip(args, self.args[key]):
                if name:
                    skwargs[name] = fixups.to_python(
                        type(sender).__name__, kwargs["signal"], name, arg, signature
                    )
                else:
    ...
    def handle_restart(self, name, old, new):
        ...
        time.sleep(
            1
        )  # Give NetworkManager a bit of time to start and rediscover itself.
...
class NMDbusInterfaceType(type):
    ...
    def __new__(type_, name, bases, attrs):
            ...
            for base in bases:
                if hasattr(base, "interface_names"):
                    attrs["interface_names"] = [
                        "%s.%s" % (base.interface_names[0], name)
        ...
        if "object_path" in attrs and attrs["object_path"]:
            ...
            attrs["introspection_data"] = proxy.Introspect(
            ...
            for element in root:
                if (
                    ...
                    and element.attrib["name"] in attrs["interface_names"]
                ):
                    for item in element:
                        if item.tag == "property":
                            attrs[item.attrib["name"]] = type_.make_property(
                            ...
                            if aname in attrs:
                                aname = "_" + aname
                            attrs[aname] = type_.make_method(
                                name, element.attrib["name"], item.attrib, list(item)
                            )
                        elif item.tag == "signal":
                            SignalDispatcher.args[
                                (element.attrib["name"], item.attrib["name"])
                            ] = [
                                (arg.attrib.get("name", None), arg.attrib["type"])
                                for arg in item
                            ]
                            attrs["On" + item.attrib["name"]] = type_.make_signal(
                                name, element.attrib["name"], item.attrib
                            )
    ...
    def make_property(klass, interface, attrib):
        ...
        def get_func(self):
            try:
                data = self.proxy.Get(
                    interface, name, dbus_interface="org.freedesktop.DBus.Properties"
                )
    ...
    def make_signal(klass, interface, attrib):
        ...
        code += (
            "    SignalDispatcher.add_signal_receiver('%s', '%s', self, func, list(args), kwargs)"
            % (interface, name)
        )
        exec(code, globals(), ret)
        return ret["On" + name]
...
class NMDbusInterface(object):
    ...
    def __new__(klass, object_path=None):
        ...
        if object_path and not klass.introspection_data:
            ...
            klass.introspection_data = proxy.Introspect(
                dbus_interface="org.freedesktop.DBus.Introspectable"
            )
            ...
            for element in root:
                if (
                    element.tag == "interface"
                    and element.attrib["name"] in klass.interface_names
                ):
                    for item in element:
                        if item.tag == "property":
                            setattr(
                                klass,
                                item.attrib["name"],
                                type(klass).make_property(
                                    klass.__name__, element.attrib["name"], item.attrib
                                ),
                            )
    ...
    def __eq__(self, other):
        return (
            isinstance(other, NMDbusInterface)
            and self.object_path
    ...
    def proxy(self):
        if not self._proxy:
            self._proxy = dbus.SystemBus().get_object(
                self.dbus_service, self.object_path
            )
...
class SecretAgent(dbus.service.Object):
    ...
    def GetSecrets(self, connection, connection_path, setting_name, hints, flags):
        settings = fixups.to_python(
            "SecretAgent", "GetSecrets", "connection", connection, "a{sa{sv}}"
        )
        connection = fixups.to_python(
            "SecretAgent", "GetSecrets", "connection_path", connection_path, "o"
        )
        setting_name = fixups.to_python(
            "SecretAgent", "GetSecrets", "setting_name", setting_name, "s"
        )
...
class fixups(object):
    ...
    def to_python(klass, method, arg, val, signature):
        ...
        klass_af = {"IP4Config": socket.AF_INET, "IP6Config": socket.AF_INET6}.get(
            klass, socket.AF_INET
        )
        if method == "Get":
            if arg == "Ip4Address":
        ...
        if method == "GetSettings":
            if "ssid" in val.get("802-11-wireless", {}):
                val["802-11-wireless"]["ssid"] = fixups.ssid_to_python(
                    val["802-11-wireless"]["ssid"]
                )
            for key in val:
                val_ = val[key]
                ...
                if "cloned-mac-address" in val_:
                    val_["cloned-mac-address"] = fixups.mac_to_python(
                        val_["cloned-mac-address"]
                    )
                if "bssid" in val_:
                    val_["bssid"] = fixups.mac_to_python(val_["bssid"])
            if "ipv4" in val:
                val["ipv4"]["addresses"] = [
                    fixups.addrconf_to_python(addr, socket.AF_INET)
                    for addr in val["ipv4"]["addresses"]
                ]
                val["ipv4"]["routes"] = [
                    fixups.route_to_python(route, socket.AF_INET)
                    for route in val["ipv4"]["routes"]
                ]
                val["ipv4"]["dns"] = [
                    fixups.addr_to_python(addr, socket.AF_INET)
                    for addr in val["ipv4"]["dns"]
                ]
            if "ipv6" in val:
                val["ipv6"]["addresses"] = [
                    fixups.addrconf_to_python(addr, socket.AF_INET6)
                    for addr in val["ipv6"]["addresses"]
                ]
                val["ipv6"]["routes"] = [
                    fixups.route_to_python(route, socket.AF_INET6)
                    for route in val["ipv6"]["routes"]
                ]
                val["ipv6"]["dns"] = [
                    fixups.addr_to_python(addr, socket.AF_INET6)
                    for addr in val["ipv6"]["dns"]
                ]
            return val
    ...
    def base_to_python(val):
        ...
        if isinstance(val, (dbus.Dictionary, dict)):
            return dict(
                [
                    (fixups.base_to_python(x), fixups.base_to_python(y))
        ...
        if isinstance(val, dbus.ObjectPath):
            ...
            if val.startswith("/org/freedesktop/NetworkManager/"):
                ...
                classname = {"Settings": "Connection", "Devices": "Device",}.get(
                    classname, classname
                )
